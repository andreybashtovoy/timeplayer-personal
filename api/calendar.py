...
async def get_activities(request):
    ...
    for activity in activities:
        results.append({
            ...
            'start_time': activity.start_time.replace(tzinfo=tz.UTC).astimezone(local_tz).timestamp() * 1000,
            'end_time': (activity.start_time.replace(tzinfo=tz.UTC).astimezone(local_tz) + activity.duration).timestamp() * 1000,
